        ...
        return (
          ...
          >
            ...
            <div className={reversed ? "lg:order-1" : undefined}>
              <div className="mb-4 inline-flex h-11 w-11 items-center justify-center border border-brand-gold/50 text-brand-gold">
                <Icon className="h-5 w-5" aria-hidden />
              </div>
              <h2 className="font-display text-3xl font-bold text-brand-ink md:text-4xl">
                {service.name}
              </h2>
              <p className="mt-4 text-base leading-relaxed text-brand-body">{service.description}</p>
              ...
              <Button href={`/quote?service=${encodeURIComponent(service.name)}`} className="mt-8">
                Request a Quote
              </Button>
            </div>
          </article>
